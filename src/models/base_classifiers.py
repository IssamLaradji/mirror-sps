...
def get_classifier(clf_name, train_set):   
    batch = train_set[0]
    d = batch['images'].shape[0] 
    if clf_name in ["linear", "logistic"]:
        
        model = Mlp_model(input_size=d, hidden_sizes=[], n_classes=2, bias=False)
    ...
    if clf_name == "mlp_dropout":
        ...
        model = LinearNetwork(d, [1], 10, bias=False)
        
    elif clf_name == "matrix_fac_4":
        model = LinearNetwork(d, [4], 10, bias=False)
    
    elif clf_name == "matrix_fac_10":
        model = LinearNetwork(d, [10], 10, bias=False)

    elif clf_name == "linear_fac":
        model = LinearNetwork(d, [], 10, bias=False)
